import { sourcePose } from './middleware-trace.js';

// Both views consume one trace cursor. Only the evaluator reference is a
// continuous curve: the reader's retained position is an exact held sample.
...
export function createMiddlewareView(container, { onModeChange = () => {} } = {}) {
  let recording, frame, mode = '2d', world, loading = false, failed = false, disposed = false, cameraMode = 'yard';
  const svg = svgNode('svg', { viewBox: '0 0 780 480', class: 'middleware-svg', role: 'img', 'aria-label': 'Top-down synthetic trajectory. A solid drone shows evaluator reference; an amber ghost shows the reader’s last received position. Text labels give altitude.' });
  const layer = Object.assign(document.createElement('div'), { className: 'middleware-three', hidden: true });
  ...
  function drawSvg() {
    ...
    const reader = frame, sx = x => 390 + x * 79, sy = y => 224 - y * 79;
    ...
    svg.replaceChildren();
    const defs = svgNode('defs'), marker = svgNode('marker', { id: 'middleware-difference-arrow', viewBox: '0 0 10 10', refX: 8, refY: 5, markerWidth: 6, markerHeight: 6, orient: 'auto-start-reverse' });
    ...
    svg.append(polyline(samples(frame.timeMs), { stroke: '#93d4b4', 'stroke-width': 2.5, opacity: .85, 'data-middleware-reference-trail': '' }));
    svg.append(svgNode('text', { x: 721, y: sy(0) - 9, fill: '#b9d0b9', 'font-size': 12 }, 'x →'), svgNode('text', { x: sx(0) + 8, y: 36, fill: '#b9d0b9', 'font-size': 12 }, 'y ↑'));
    if (reader.position) {
      svg.append(line(reader.position, frame.sourcePosition, { stroke: '#e9aaa0', 'stroke-width': 2, 'stroke-dasharray': '5 4', 'marker-end': 'url(#middleware-difference-arrow)', 'data-middleware-difference': '' }));
      const [x, y, z] = reader.position, ghost = svgNode('g', { transform: `translate(${sx(x)} ${sy(y)})`, 'data-middleware-svg-retained': '', 'data-position': JSON.stringify(reader.position) });
      ...
      svg.append(svgNode('rect', { x: labelX - 157, y: labelY - 16, width: 314, height: 22, rx: 3, fill: '#3b3528ed' }), svgNode('text', { x: labelX, y: labelY, 'text-anchor': 'middle', fill: '#f4cea0', 'font-size': 11 }, `retained #${reader.lastCallback.seq} · z ${z.toFixed(2)} m · age ${reader.generationAgeMs.toFixed(0)} ms`));
    }
    const [x, y, z] = frame.sourcePosition, drone = svgNode('g', { transform: `translate(${sx(x)} ${sy(y)})`, 'data-middleware-svg-source': '', 'data-position': JSON.stringify(frame.sourcePosition) });
    ...
    svg.append(svgNode('text', { x: 28, y: 462, fill: '#b7cdb9', 'font-size': 10 }, 'SYNTHETIC REFERENCE CONTINUES · GRID = 1 M · RECORDED CALLBACKS · NO FLIGHT PHYSICS'));
  }
  ...
  async function prepareThree() {
    ...
    loading = true;
    layer.replaceChildren(Object.assign(document.createElement('p'), { className: 'middleware-webgl-message', textContent: 'Preparing the late-join telemetry yard…' }));
    let pendingRenderer;
    try {
      ...
      renderer.domElement.setAttribute('aria-label', '3D synthetic telemetry yard. Solid quadrotor: evaluator reference. Amber wireframe quadrotor: reader’s retained XYZ sample. Drag to orbit, scroll to zoom, focus and use arrow keys to pan.');
      ...
      // The stationary reader console shows application observation only.
      const consoleUnit = new THREE.Group(); consoleUnit.position.set(0, 0, 3.35); scene.add(consoleUnit);
      box([1.7, .8, .6], mat(0x455e53), [0, .4, 0], consoleUnit, 'late-reader-console');
      box([1.9, .09, .8], metal, [0, .84, 0], consoleUnit);
      const readerScreen = mat(0x82928b, { emissive: 0x344b40, emissiveIntensity: .4, roughness: .4 });
      box([1.35, .4, .025], dark, [0, .51, .319], consoleUnit);
      box([1.16, .26, .035], readerScreen, [0, .51, .339], consoleUnit);
      for (let i = 0; i < 5; i++) box([.035, .04, .04], pale, [-.45 + .225 * i, .25, .339], consoleUnit);
      ...
      const overlay = Object.assign(document.createElement('div'), { className: 'middleware-labels' });
      const sourceLabel = Object.assign(document.createElement('span'), { className: 'middleware-scene-label' }), ghostLabel = Object.assign(document.createElement('span'), { className: 'middleware-scene-label middleware-memory' }); overlay.append(sourceLabel, ghostLabel); layer.append(overlay);
      const cameras = Object.assign(document.createElement('div'), { className: 'middleware-camera' }); cameras.setAttribute('role', 'group'); cameras.setAttribute('aria-label', '3D camera framing');
      const yardButton = Object.assign(document.createElement('button'), { id: 'middleware-camera-yard', textContent: 'Whole yard' }), closeButton = Object.assign(document.createElement('button'), { id: 'middleware-camera-close', textContent: 'Follow reference' }); cameras.append(yardButton, closeButton); layer.append(cameras);
      layer.append(Object.assign(document.createElement('span'), { className: 'middleware-scene-caption', textContent: 'SYNTHETIC XYZ REFERENCE · GHOST MOVES ONLY ON RECORDED CALLBACKS' }));
      world = { THREE, renderer, scene, camera, controls, drone, rotors, ghost, reference, trail, vertical, ghostVertical, difference, sourceRing, ghostRing, sourceLabel, ghostLabel, readerScreen, yardButton, closeButton, referenceRun: null };
  ...
  function updateThree() {
    ...
    const reader = frame, { THREE } = world;
    ...
    const screenColor = frame.lastCallback ? (frame.lastCallback.generatedMs < recording.config.joinMs ? 0xd1ac6d : 0x92d2ad) : 0x718479;
    world.readerScreen.color.setHex(screenColor); world.readerScreen.emissive.setHex(screenColor);
    ...
    world.difference.visible = Boolean(reader.position && Math.hypot(...frame.sourcePosition.map((axis, i) => axis - reader.position[i])) > .025); world.ghostLabel.hidden = !reader.position;
    if (reader.position) {
      const remembered = worldPoint(reader.position), rt = reader.lastCallback.generatedMs / 1000;
      ...
      world.ghostLabel.textContent = `Held #${reader.lastCallback.seq} · ${(reader.generationAgeMs / 1000).toFixed(2)} s old`;
    }
  ...
  function drawThree() {
    ...
    const width = container.clientWidth, height = container.clientHeight, reader = frame;
    const occupied = [];
  ...
  return {
    update(nextRecording, nextFrame) {
      recording = nextRecording; frame = nextFrame;
      const reader = frame;
      Object.assign(container.dataset, { case: recording.id, timeMs: String(frame.timeMs), sourcePosition: JSON.stringify(frame.sourcePosition), retainedPosition: JSON.stringify(reader.position), readerStatus: frame.readerStatus, phase: frame.phase, rmw: recording.rmw, durability: recording.durability, view: mode });
      drawSvg(); updateThree();
    },
    setMode(next) {
      if (disposed) return;
