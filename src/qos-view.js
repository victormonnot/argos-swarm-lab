import { sourcePose } from './qos-trace.js';

// Both views consume one trace cursor. Only the evaluator reference is a
// continuous curve: a reader's remembered position is an exact held sample.
...
export function createQosView(container, { onModeChange = () => {} } = {}) {
  let recording, frame, selected, mode = '2d', world, loading = false, failed = false, disposed = false, cameraMode = 'yard';
  const svg = svgNode('svg', { viewBox: '0 0 780 480', class: 'qos-svg', role: 'img', 'aria-label': 'Top-down synthetic trajectory. A solid drone shows evaluator reference; an amber ghost shows the selected reader’s last accepted position. Text labels give altitude.' });
  const layer = Object.assign(document.createElement('div'), { className: 'qos-three', hidden: true });
  ...
  function drawSvg() {
    ...
    const reader = frame.readers.find(item => item.id === selected), sx = x => 390 + x * 79, sy = y => 224 - y * 79;
    ...
    svg.replaceChildren();
    const defs = svgNode('defs'), marker = svgNode('marker', { id: 'qos-difference-arrow', viewBox: '0 0 10 10', refX: 8, refY: 5, markerWidth: 6, markerHeight: 6, orient: 'auto-start-reverse' });
    ...
    svg.append(polyline(samples(frame.timeMs), { stroke: '#93d4b4', 'stroke-width': 2.5, opacity: .85, 'data-qos-reference-trail': '' }));
    svg.append(svgNode('text', { x: 721, y: sy(0) - 9, fill: '#b9d0b9', 'font-size': 12 }, 'x →'), svgNode('text', { x: sx(0) + 8, y: 36, fill: '#b9d0b9', 'font-size': 12 }, 'y ↑'));
    if (reader.position) {
      svg.append(line(reader.position, frame.sourcePosition, { stroke: '#e9aaa0', 'stroke-width': 2, 'stroke-dasharray': '5 4', 'marker-end': 'url(#qos-difference-arrow)', 'data-qos-difference': '' }));
      const [x, y, z] = reader.position, ghost = svgNode('g', { transform: `translate(${sx(x)} ${sy(y)})`, 'data-qos-svg-retained': '', 'data-position': JSON.stringify(reader.position) });
      ...
      svg.append(ghost);
      const labelX = Math.max(140, Math.min(640, sx(x))), labelY = Math.min(417, sy(y) + 49);
      svg.append(svgNode('rect', { x: labelX - 131, y: labelY - 16, width: 262, height: 22, rx: 3, fill: '#3b3528ed' }), svgNode('text', { x: labelX, y: labelY, 'text-anchor': 'middle', fill: '#f4cea0', 'font-size': 11 }, `retained #${reader.lastAccepted.seq} · z ${z.toFixed(2)} m · age ${reader.ageMs.toFixed(0)} ms`));
    }
    const [x, y, z] = frame.sourcePosition, drone = svgNode('g', { transform: `translate(${sx(x)} ${sy(y)})`, 'data-qos-svg-source': '', 'data-position': JSON.stringify(frame.sourcePosition) });
    ...
    svg.append(svgNode('text', { x: 28, y: 462, fill: '#b7cdb9', 'font-size': 10 }, 'SYNTHETIC XYZ · TOP-DOWN X/Y · GRID = 1 M · GHOST = HELD SAMPLE'));
  }
  ...
  async function prepareThree() {
    ...
    loading = true;
    layer.replaceChildren(Object.assign(document.createElement('p'), { className: 'qos-webgl-message', textContent: 'Preparing the synthetic telemetry yard…' }));
    let pendingRenderer;
    try {
      ...
      renderer.domElement.setAttribute('aria-label', '3D synthetic telemetry yard. Solid quadrotor: evaluator reference. Amber wireframe quadrotor: selected reader’s retained XYZ sample. Drag to orbit, scroll to zoom, focus and use arrow keys to pan.');
      ...
      const ghost = new THREE.Group(); ghost.name = 'reader-held-position-quadrotor'; scene.add(ghost);
      ...
      const overlay = Object.assign(document.createElement('div'), { className: 'qos-labels' });
      const sourceLabel = Object.assign(document.createElement('span'), { className: 'qos-scene-label' }), ghostLabel = Object.assign(document.createElement('span'), { className: 'qos-scene-label qos-memory' }); overlay.append(sourceLabel, ghostLabel); layer.append(overlay);
      const cameras = Object.assign(document.createElement('div'), { className: 'qos-camera' }); cameras.setAttribute('role', 'group'); cameras.setAttribute('aria-label', '3D camera framing');
      const yardButton = Object.assign(document.createElement('button'), { id: 'qos-camera-yard', textContent: 'Whole yard' }), closeButton = Object.assign(document.createElement('button'), { id: 'qos-camera-close', textContent: 'Follow reference' }); cameras.append(yardButton, closeButton); layer.append(cameras);
      layer.append(Object.assign(document.createElement('span'), { className: 'qos-scene-caption', textContent: 'SYNTHETIC XYZ · NO FLIGHT PHYSICS · GHOST = LAST ACCEPTED SAMPLE' }));
      world = { THREE, renderer, scene, camera, controls, drone, rotors, ghost, reference, trail, vertical, ghostVertical, difference, sourceRing, ghostRing, sourceLabel, ghostLabel, yardButton, closeButton, referenceRun: null };
  ...
  function updateThree() {
    ...
    const reader = frame.readers.find(item => item.id === selected), { THREE } = world;
    ...
    if (reader.position) {
      ...
      world.ghostLabel.textContent = `Retained #${reader.lastAccepted.seq} · z ${reader.position[2].toFixed(2)} m · ${reader.ageMs.toFixed(0)} ms old`;
    }
  ...
  function drawThree() {
    ...
    const width = container.clientWidth, height = container.clientHeight, reader = frame.readers.find(item => item.id === selected);
    const occupied = [];
  ...
  return {
    update(nextRecording, nextFrame, nextSelected) {
      ...
      const reader = frame.readers.find(item => item.id === selected);
      Object.assign(container.dataset, { case: recording.id, timeMs: String(frame.timeMs), readerId: selected, sourcePosition: JSON.stringify(frame.sourcePosition), retainedPosition: JSON.stringify(reader.position), fresh: String(reader.fresh), view: mode });
      drawSvg(); updateThree();
    },
    setMode(next) {
      if (disposed) return;
