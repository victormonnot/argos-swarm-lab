...
test('keyboard selection, mobile layout and all workshop links remain usable', async ({ page }) => {
  ...
  for (const [name, id] of [['01 / Average consensus', '#step-count'], ['02 / Potential fields', '#movement-step-count'], ['03 / Task allocation', '#mission-step-count'], ['04 / Decision architectures', '#arch-step-count'], ['05 / A* path planning', '#path-step-count'], ['06 / Kalman position filtering', '#loc-step-count'], ['07 / Shared estimates', '#fusion-round'], ['08 / ORCA collision avoidance', '#orca-step-count'], ['09 / CBBA task bundles', '#cbba-round'], ['10 / Behavior Trees and FSM', '#behavior-step-count'], ['11 / Cooperative localization', '#coop-step-count'], ['12 / EKF-SLAM', '#slam-step-count'], ['13 / Pose-graph SLAM', '#graph-iteration'], ['14 / ROS 2 nodes and topics', '#ros-round'], ['15 / Message freshness and QoS', '#qos-time-label'], ['16 / Process failure and restart', '#restart-time'], ['17 / Fast DDS and Zenoh', '#middleware-time'], ['18 / ArduPilot SITL and MAVLink', '#sitl-time'], ['19 / Gazebo external physics', '#gazebo-time'], ['20 / GCS heartbeat failsafe', '#failsafe-time'], ['21 / Two-vehicle mission', '#fleet-time'], ['22 / Mission recovery', '#recovery-time'], ['23 / Shared Gazebo world', '#shared-world-time']]) {
    await page.getByRole('navigation', { name: 'Workshops', exact: true }).getByRole('link', { name, exact: true }).click();
    await expect(page.locator(id)).toHaveText(['#qos-time-label', '#restart-time', '#middleware-time', '#sitl-time', '#gazebo-time', '#failsafe-time', '#fleet-time', '#recovery-time', '#shared-world-time'].includes(id) ? '0.000 s' : '0');
  }
});
